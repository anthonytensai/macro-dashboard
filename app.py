...
with sb2:
    st.metric("Regime", regime.replace("Late-Cycle Narrow Rally","Late-Cycle").replace("Goldilocks Risk-On","Goldilocks").replace("Neutral/Transition","Neutral"))
with sb3:
...
with sb5:
    st.metric("SPY", f"${spy_now:,.0f}" if not np.isnan(spy_now) else "—",
              delta=f"{spy_1m:+.1f}% 1M {'✅ above 200DMA' if above_200 else '⚠️ below 200DMA'}" if not np.isnan(spy_1m) else None)
with sb6:
...
# Second row — action + gross + agreement
sb8, sb9, sb10 = st.columns(3)
with sb8:
    action_colors = {"Add": "🟢", "Add (selective)": "🟢", "Hold": "🟡", "Trim": "🟠", "Deleverage": "🔴"}
    icon = action_colors.get(sizing.action, "◆")
    st.metric("Action", f"{icon} {sizing.action}")
with sb9:
    st.metric("Target Gross", f"{sizing.gross_low:.2f}x – {sizing.final_gross_high:.2f}x",
              delta=f"max {sizing.max_gross:.2f}x")
with sb10:
    st.metric("Signal Agreement", f"▲{n_bull}B · ◆{n_neut}N · ▼{n_bear}Bear")
